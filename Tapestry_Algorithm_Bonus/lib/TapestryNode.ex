defmodule TapestryNode do
  ...
  def handle_cast({:next_hop, hopCount, level, objectId}, state) do
    ...
    IO.puts "Hey in next hop"
    IO.inspect next_hop
    if Enum.at(next_hop["node"],0) == self do
      ...
      GenServer.cast(String.to_atom("actor_"<>Enum.at(next_hop["node"],0)), {:next_hop, hopCount + 1, next_hop["level"], objectId})
    end
    {:noreply, state}
  end
